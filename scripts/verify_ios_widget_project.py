...
require("flutter test --no-pub --concurrency=1" in workflow, "release workflow runs Flutter tests sequentially without resolving dependencies again")
...
require("flutter build ios --release --config-only" in workflow, "release workflow generates Flutter iOS configuration without invoking signing")
